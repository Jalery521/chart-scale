...
exports.getChartScale = exports.formatNumInteger = void 0;
 ...
 */
function formatNumInteger(number) {
    var power = 0;
...
exports.formatNumInteger = formatNumInteger;
var magic = [10, 15, 20, 25, 30, 40, 50, 60, 70, 80, 90, 100, 120, 150]; //魔数数组经过扩充，放宽魔数限制避免出现取不到魔数的情况。
/**
 ...
 */
function getChartScale(maxNum, minNum, splitNumber) {
    maxNum = maxNum || 0;
    minNum = minNum || 0;
    if (maxNum === minNum) {
        // 如果都为0则不进行计算直接返回
    ...
    var maxValue = (0, bignumber_js_1.default)(maxNum);
    var minValue = (0, bignumber_js_1.default)(minNum);
    //计算出初始间隔tempGap和缩放比例multiple
    var _a = formatNumInteger(maxValue.minus(minValue).toNumber()), formatNumber = _a.formatNumber, power = _a.power;
    var remainder = formatNumber % splitNumber;
    var rangeLength;
    if (remainder !== 0) {
        rangeLength = (0, bignumber_js_1.default)(formatNumber).plus(splitNumber).minus(remainder);
    }
    rangeLength = (0, bignumber_js_1.default)(10).pow(power).times(formatNumber);
    var tempGap = rangeLength.div(splitNumber); //初始刻度间隔的大小。
    //设tempGap除以multiple后刚刚处于魔数区间内，先求multiple的幂10指数，例如当tempGap为120，想要把tempGap映射到魔数数组（即处理为10到100之间的数），则倍数为10，即10的1次方。
    var multipleI = Math.floor(Math.log10(tempGap.toNumber()) - 1); //这里使用Math.floor的原因是，当Math.log10(tempGap)-1无论是正负数都需要向下取整。不能使用parseInt或其他取整逻辑代替。
    //刚才是求出指数，这里求出multiple的实际值。
    var multiple = (0, bignumber_js_1.default)(10).pow(multipleI); // 解决Math.pow(10,-4)得到0.00009999999999999999的问题
    //取出邻近较大的魔数执行第一次计算
    // const tempStep = tempGap / multiple
    var tempStep = tempGap.div(multiple); //映射后的间隔大小
    var estep = (0, bignumber_js_1.default)(0); //期望得到的间隔
    ...
    var max = (0, bignumber_js_1.default)(0);
    var min = (0, bignumber_js_1.default)(0);
    ...
    if (max.comparedTo(0) === 0) {
        ...
        var shouldMin = estep.times(-splitNumber);
        ...
        var shouldMax = estep.times(splitNumber);
        ...
        outter: do {
            ...
            if ((i - lastIndex) * (tempSplitNumber - splitNumber) < 0) {
                ...
                while (tempSplitNumber < splitNumber) {
                    ...
                    if (([0, -1].includes(min.minus(minValue).comparedTo(max.minus(maxValue))) &&
                        min.comparedTo(0) !== 0) ||
                        max.comparedTo(0) === 0) {
    ...
    var intervalNum = max.minus(min).div(splitNumber).toNumber();
    // 处理interval格式(避免刻度间隔小数位长度过长)
    var _b = formatNumInteger(intervalNum), formatInterval = _b.formatNumber, intervelPower = _b.power;
    var interval = (0, bignumber_js_1.default)(formatInterval).times((0, bignumber_js_1.default)(10).pow(intervelPower));
...
exports.getChartScale = getChartScale;
exports.default = getChartScale;
